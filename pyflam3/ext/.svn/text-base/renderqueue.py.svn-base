...
def progress_proc(params, progress, stage):
    queue, job = params
...
class RenderJob(object):
    ...
    def __init__(self, genome, cancel_cb=None, completed_cb=None, 
    ...
    def _execute(self, **kwargs):
        ...
        try:
            self.buffer, self.stats = self.genome.render(**self.args)
        except Exception, e:
...
class RenderQueue(threading.Thread):
    ...
    def run(self):
        while 1:
            ...
            if not job._execute(
                    progress_func=pyflam3.ProgressFunction(progress_proc),
                    progress_param=(self,job)):
